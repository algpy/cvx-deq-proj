...
def init_fft_conv(weight, hw):
    ...
    return fft_to_complex_matrix(torch.fft.fft2(kernel))
...
def fft_conv(x, w_fft, transpose=False):
    ...
    """
    x_fft = fft_to_complex_vector(torch.fft.fft2(x))
...
class MONMultiConv(nn.Module):
    ...
    def apply_inverse_conv(self, z, i):
        z0_fft = fft_to_complex_vector(torch.fft.ifft2(z))
    ...
    def apply_inverse_conv_transpose(self, g, i):
        g0_fft = fft_to_complex_vector(torch.fft.fft2(g))
